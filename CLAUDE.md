...
38. The **public** scan is static HTTP/HTML inspection and stays that way — no browser automation, no headless browser, in `src/modules/live-product-intelligence/` (see [ADR 0010](docs/decisions/0010-safe-outbound-http-inspection.md), and rules 36–37 and 39 either side of this one). Elsewhere the prohibition has been lifted by decision rather than by drift, and this rule names the two so nobody reads it as a ban that HEAD contradicts: [ADR 0012](docs/decisions/0012-authenticated-browser-analysis.md) admits a real browser for **authenticated** analysis, which no static fetch can reach, and it is now the only such admission — [ADR 0017](docs/decisions/0017-visual-review-artifacts.md) built visual review on the same one, and [ADR 0075](docs/decisions/0075-the-photograph-nobody-took.md) deleted that path, so nothing in `src/modules/review/` opens a browser. Browserbase is gone too: [ADR 0076](docs/decisions/0076-the-browser-we-own.md) runs the Deep Scan's browser as Chromium inside a Vercel sandbox Vibe creates. So `playwright-core` is the only browser package this repository depends on and it is a production dependency on purpose, and Chromium is admitted **there and only there** — installed into a sandbox image, never into Vibe's own runtime. A *further* use — Puppeteer, Browserless, Firecrawl, Apify, a headless browser anywhere in a Vibe process, or any of these in a new place — is still a new ADR and not a new dependency.
...
41. Never grant an inference call tools, web search, URL fetching, code execution, or repository/database access. Evidence reaching a model is untrusted data; removing capabilities — not prompt wording — is what bounds prompt injection. There are exactly **two** exceptions, each bounded by its own named triple, and a third is a new ADR. **Agentic execution** is bounded by an isolated VM holding no credential, an explicitly named tool set with no network tool, and Vibe's own verification of the result (rules 75–82). **The Business Agent's turn** is bounded by [ADR 0109](docs/decisions/0109-the-business-agent-is-the-orchestrator.md): a closed `as const` tool set of reads and preparations, so the capability is *absent* and there is nothing to grant or revoke (rule 76); every identifier the model supplies resolved server-side against the project's own rows, with the project id taken from the persisted operation row and never from the model (rule 53), because an argument is a request and the runtime's check is the fact; and every reply validated deterministically before a founder reads it — no banned or causal claim, no number absent from that turn's tool results, no artifact reference a tool did not return (rule 45) — a refused reply replaced by a template and recorded as refused. That exception admits **no** tool that writes to a repository, starts a paid operation, approves, merges or deploys anything, moves money or spends Credits, opens a connection to any URL, runs a shell command, composes SQL, or names a provider or a model. Each of those is absent, not denied. A consequential action is a control the founder presses at its stated price (rule 60, rule 67, rule 70), never a tool the model holds. The tool set, its ceilings and its reply validation live in code, never in prompt prose, and the loop lives in the domain module — never in the provider, which performs exactly one turn per call. Neither exception is a licence to give a third model a tool.
42. Instructions to a model come only from prompts we author. Never interpolate repository, website, or user content into a system prompt; third-party content belongs in a fenced, untrusted-labelled user message. A founder's own message and every tool result are third-party content by this rule: they are fenced and labelled with their source in the user turn, never merged into the system prompt, and a tool result that reads like an instruction is still data (rule 25, rule 36). The tool catalogue and the skills are prompts we author, so they belong in the system prompt; nothing derived from a customer's repository, site or message ever becomes one.
43. Never request, persist, or display model reasoning. Store validated structured conclusions, short rationale, and evidence references only. Reasoning token counts may be recorded because they are billed. A tool-calling turn stores the same way: the tool calls Vibe dispatched, the results Vibe produced, and the validated assistant message the founder reads are the turn's conclusions and are persisted; thinking blocks are dropped at the adapter on both contracts and never cross a step boundary, a database row or a screen.
...
47. Count tokens before every paid call and record a usage event after it, for successes and failures alike — but only when tokens were genuinely billed. Never store prompt text, model responses, reasoning, or secrets in the usage ledger. "Every paid call" counts each call of a loop, not each turn: a turn that calls the model six times counts six times and records six rows, with cached and uncached input reported separately because they are billed at different rates. A turn that stops at a ceiling still records what it spent reaching it.
...
78. An agent's own checks are advisory; Vibe's independent validation is the verdict. Never let a run self-certify, and never activate a customer-facing Agent price without a measured cost behind it. That bar was met: `launch-v1` prices `agent_execution` from sixteen delivered dogfood runs, and [ADR 0092](docs/decisions/0092-the-agent-runs-as-the-product.md) then removed the internal ceiling and the operator allowlist that reached it — which was also gating the start path, so no customer could run the agent at all. There is one book now, the customer's: every project can start a run and every run is charged at retail. Never reintroduce a second economics world, and never gate a customer capability on an environment variable nothing documents.
