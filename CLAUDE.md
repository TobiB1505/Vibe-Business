...
38. The **public** scan is static HTTP/HTML inspection and stays that way — no browser automation, no headless browser, in `src/modules/live-product-intelligence/` (see [ADR 0010](docs/decisions/0010-safe-outbound-http-inspection.md), and rules 36–37 and 39 either side of this one). Elsewhere the prohibition has been lifted by decision rather than by drift, and this rule names the two so nobody reads it as a ban that HEAD contradicts: [ADR 0012](docs/decisions/0012-authenticated-browser-analysis.md) admits Browserbase and Playwright for **authenticated** analysis, which no static fetch can reach, and it is now the only one — [ADR 0017](docs/decisions/0017-visual-review-artifacts.md) built visual review on the same admission, and [ADR 0074](docs/decisions/0074-the-photograph-nobody-took.md) deleted that path, so nothing in `src/modules/review/` opens a browser. `@browserbasehq/sdk` and `playwright-core` are therefore production dependencies on purpose, for the Deep Scan alone. A *third* use — Puppeteer, Chromium, Browserless, Firecrawl, Apify, or any of these in a new place — is still a new ADR and not a new dependency.
