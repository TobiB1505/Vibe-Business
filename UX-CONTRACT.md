...
| Agent workspace | five named stages present at first paint, marks and words per stage | one core breath and one orbiting signal, only while working, paused when the tab is hidden | rail is a labelled `nav`; every stage is announced with its position and state | readiness card instead of a stepper when nothing has ever run | named stages, no percentage, no connector that fills part way; panel geometry reserved | failed and skipped stages differ from pending in mark, label and words |
...
| Run with Vibe | `Start with Vibe` on the Action Plan or `Run with Vibe` in the Agent for the exact selected Move | confirmation/admission first, then stable busy action | that Move's run and prepared change on `/agent` | persistent inline failure with retry only where honest | submitted action remains contextual | ADR 0014 |
...
- The Agent workspace stacks to one column below `lg` and puts the stage rail and the current stage's words above the core: on a narrow screen a founder must not scroll past an animation to find out what is happening. Stage marks, labels and state words remain fully readable at every width.
