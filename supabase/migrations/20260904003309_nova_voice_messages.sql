-- One Nova voice message per identity, attempted exactly once (ADR 0084).
--
...
comment on table public.nova_voice_messages is
  'One Nova voice message per reuse identity, attempted exactly once (ADR 0084). Claimed before the provider call; never retried. A fallback row stores its reason and no text, because the fallback text is the deterministic Vibe template the caller already holds.';
