...
Status: Accepted; its **account-dashboard clause** — that the account level is a screen of its own — is superseded by [ADR 0104](0104-the-account-level-is-settings.md) on 2026-09-07. `/app` resolves to a product and renders nothing. What this ADR decided about the *context swap* — that the sidebar changes what it is about rather than the application changing — stands, and is what makes a Settings area with its own rail readable rather than a second product. Its **two-rails clause** — that the two rails are mutually exclusive by construction, one per route-group layout — is superseded by [ADR 0105](0105-one-rail-that-unfolds.md) on 2026-09-07: the construction that guarantees exclusivity also guarantees the rail cannot survive the move between the two, so there is one `<aside>` in the shared layout and the navigation inside it is a parallel route. The context swap itself is unchanged; it is now a fold rather than a replacement.

Date: 2026-08-23

**Visual amendment:** [UI-8](../sprints/0086-account-dashboard-reference-fidelity.md) later
...
**Products-index amendment:** [UI-9](../sprints/0087-my-products-reference-fidelity.md) makes
...
[ADR 0045](0045-command-center-information-architecture.md) rebuilt the *project*
