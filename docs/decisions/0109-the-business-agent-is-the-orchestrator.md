...
**No provider credential was available to the session that built this** — no `ANTHROPIC_API_KEY`, no auth token, no CLI profile — so the paid comparison the instrument exists to run was not run. What follows is what the instrument measures without a provider, from the real prompt, descriptors, schema and fixture (`measure.test.ts` prints it on every run):
