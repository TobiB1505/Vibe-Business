# 0099 - Glass is the material, and the ground has to be visible for it to be one

Status: Accepted
Date: 2026-09-07

Supersedes the "glass is emphasis" clause of [ADR 0096](0096-the-second-design-system-arrives-scoped.md). In the second palette a `card`, a `panel` and the chrome are all panes; a `section` is the one opaque step. And the shells stop painting `--color-app` over the whole viewport, because that is what had been covering the ground the glass was drawn to refract.
