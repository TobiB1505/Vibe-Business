# 0102 - The palette ships behind one switch, not one route at a time

Status: Accepted
Date: 2026-09-07

Supersedes the rollout clause of [ADR 0097](0097-the-second-design-system-arrives-scoped.md). The second design system is chosen once, in the root layout, from `VIBE_PALETTE`. It is not migrated route by route, because the ground is a fixed layer and a half-migrated product changes its own background as a founder navigates.

## Context

ADR 0097 shipped the second palette dormant: same token names, new values, every declaration scoped to `[data-vibe="v2"]`, and nothing in the product carrying the attribute. It also wrote down how the rollout would go — "a route opts in by carrying the attribute on a layout wrapper; it opts out by deleting it."
...
ADR 0097 assumed a route could opt in on a layout wrapper. It can — the attribute is a plain CSS scope — but the *ground* cannot follow it. A fixed, full-viewport ramp belongs to the document, not to a route, so a product where half the routes are v2 has a background that switches on and off as a founder moves between screens, with the corner radii jumping at the same moment. That is worse than either end state.
...
- The variable is not the only switch, and could not be. Every screen in the redesign now has to be checked in both palettes, and a build-time flag makes that a redeploy per look. So non-production deployments carry a per-browser control in the account menu: it writes `localStorage`, a blocking script in `<head>` applies it before the first paint, and the rail says "local override" so a screenshot cannot be mistaken for the deployment. It is gated on `getAppEnvironment() !== "production"` — an existing answer to "which tier is this", so no new configuration — and it disappears with the scope. A customer offered a choice between a finished design and an unfinished one has been handed a decision that is not theirs.
- When v2 is what everybody has, the values move into `@theme`, the scope and the switch are deleted, and no compatibility layer is left behind — unchanged from ADR 0097.
