...
**Slices 0–4 have shipped** ([Sprint 0222](../sprints/0222-nova-leaves-the-route-layer.md), [0223](../sprints/0223-a-component-that-knew-the-product.md), [0224](../sprints/0224-the-surfaces-leave-the-routes.md), [0225](../sprints/0225-one-address-and-a-door-per-feature.md), [0226](../sprints/0226-an-artifact-knows-where-it-is-read.md)). §2's layering holds with no exception available: `TRANSITIONAL_CROSSINGS` in `src/lib/consistency/feature-boundaries.test.ts` is `[]`, `src/components` composes no product surface, no `"use server"` module lives under `src/app` outside two named exemptions, and one file in `src/lib/routing/` owns the project URL. §4's registry is `src/features/workspace/`, total over eight kinds, and every block in Nova's thread now names the section it is read in full at.

**What is decided and not yet built.** §4's *pane* and its bottom sheet: at 1280 the project rail and Nova's work column leave the thread about 640px, so the third column waits for §1's navigation change in Slice 7 — the audit's 2026-09-17 correction records the measurement. And **nothing of §5 or §6**: there is no thread table, no composer and no conversation path in the product, so every condition in §5 is a condition on code that does not exist yet. Every address is unchanged at HEAD. Slices 5–8 are recorded in [the audit](../audits/2026-09-16-nova-first-restructure/README.md).
