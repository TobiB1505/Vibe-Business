...
**Slices 0–6 have shipped** ([Sprint 0222](../sprints/0222-nova-leaves-the-route-layer.md), [0223](../sprints/0223-a-component-that-knew-the-product.md), [0224](../sprints/0224-the-surfaces-leave-the-routes.md), [0225](../sprints/0225-one-address-and-a-door-per-feature.md), [0226](../sprints/0226-an-artifact-knows-where-it-is-read.md), [0227](../sprints/0227-a-conversation-that-survives-a-reload.md), [0228](../sprints/0228-nova-may-be-asked.md)).

§2's layering holds with no exception available. §4's registry is `src/features/workspace/` over a closed union in `src/modules/nova/artifacts.ts`. §6 is real: `nova_threads` and `nova_messages` exist, a run's terminal transition leaves a `system` event, and `transcript-is-not-a-position.test.ts` refuses any import that would let the ranking read one. **§5 is built**: `modules/nova/conversation/` assembles a bounded pack, calls one model with no tool and no capability, validates every field against that pack and falls back to a deterministic template five ways; `modules/nova/intent/` resolves an instruction without a model at all and refuses what it is not sure of; one `"use server"` command is the only place generation can happen. `conversation-lane.test.ts` holds the seven conditions above as assertions rather than as prose. The price is [ADR 0110](0110-a-question-costs-nothing-and-is-bounded.md)'s: free, bounded, and still counted.

**Not deployed, and off by default.** Both of Slice 5's and Slice 6's migrations are proven against a real PostgreSQL cluster the test harness creates itself, and the remote database has neither: the sessions that wrote them had no `SUPABASE_ACCESS_TOKEN`, so `pnpm db:push` and `pnpm db:types` could not run, and the SQL-editor fallback is refused by rules 29–34. `src/types/database.ts` is therefore two migrations behind on purpose rather than hand-edited. The conversation lane is additionally behind `NOVA_CONVERSATION_ENABLED`, default off: every path through it answers, so switching it off is Nova saying she cannot answer rather than an outage.

**What is decided and not yet built.** §4's *pane* and its bottom sheet: at 1280 the project rail and Nova's work column leave the thread about 640px, so the third column waits for §1's navigation change in Slice 7 — the audit's 2026-09-17 correction records the measurement. And §1's navigation itself: the app level is still Products · Settings under [ADR 0104](0104-the-account-level-is-settings.md), and the project rail is still seven rows. Every address is unchanged at HEAD. Slices 7 and 8 are recorded in [the audit](../audits/2026-09-16-nova-first-restructure/README.md).
