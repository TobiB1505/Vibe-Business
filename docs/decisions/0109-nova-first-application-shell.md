...
**Deployed, and off by default.** Slice 5's and Slice 6's migrations are on the production database as of 2026-09-17; Slice 7's boundary migration is written, proven against a real PostgreSQL and **not deployed** — this environment has no `SUPABASE_ACCESS_TOKEN` and a production migration is not something to apply unasked. Until it is, a founder asking the first question in a project no run has ever finished in hits `42501`: `nova_threads` grants `authenticated` no insert, and Slice 6 calls `ensureOpenThread` under the founder's own session. The conversation lane is additionally behind `NOVA_CONVERSATION_ENABLED`, default off, and has never met a model: there is no conversation eval.
