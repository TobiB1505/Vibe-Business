...
**Slices 0–5 have shipped** ([Sprint 0222](../sprints/0222-nova-leaves-the-route-layer.md), [0223](../sprints/0223-a-component-that-knew-the-product.md), [0224](../sprints/0224-the-surfaces-leave-the-routes.md), [0225](../sprints/0225-one-address-and-a-door-per-feature.md), [0226](../sprints/0226-an-artifact-knows-where-it-is-read.md), [0227](../sprints/0227-a-conversation-that-survives-a-reload.md)). §2's layering holds with no exception available: `TRANSITIONAL_CROSSINGS` in `src/lib/consistency/feature-boundaries.test.ts` is `[]`, `src/components` composes no product surface, no `"use server"` module lives under `src/app` outside two named exemptions, and one file in `src/lib/routing/` owns every project address. §4's registry is `src/features/workspace/` over a closed union in `src/modules/nova/artifacts.ts`, and every block in Nova's thread names the section it is read in full at. §6 is real: `nova_threads` and `nova_messages` exist, a run's terminal transition leaves a `system` event, and `transcript-is-not-a-position.test.ts` refuses any import that would let the ranking read one.

**Not deployed.** Slice 5's migration is written and proven against a real PostgreSQL cluster the test harness creates itself, and the remote database does not have it: the session that built it had no `SUPABASE_ACCESS_TOKEN`, so `pnpm db:push` and `pnpm db:types` could not run, and the SQL-editor fallback is refused by rules 29–34. `src/types/database.ts` is therefore one migration behind on purpose rather than hand-edited.

**What is decided and not yet built.** §4's *pane* and its bottom sheet: at 1280 the project rail and Nova's work column leave the thread about 640px, so the third column waits for §1's navigation change in Slice 7 — the audit's 2026-09-17 correction records the measurement. And **all of §5**: there is no composer, no intent resolver and no conversation path, so every condition in §5 is a condition on code that does not exist yet. A thread today fills with what happened and offers nothing to type. Every address is unchanged at HEAD. Slices 6–8 are recorded in [the audit](../audits/2026-09-16-nova-first-restructure/README.md).
