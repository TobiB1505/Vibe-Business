...
**§1's navigation is not built, and is blocked on a decision rather than on effort.** The app level here reads *Products · New chat · Threads · Settings*, and a thread is **project-scoped** — the restructure audit's §C.9 and §E.4, and what Slice 5 built. Those cannot both hold: *Threads* at the account level names something that does not exist, and *New chat* asks which product. §E.4 (are threads also account-level?) and §E.5 (which of Threads and Account takes the phone's fourth tab) are the two answers needed, and inventing either would be inventing a product requirement. **§4's pane waits with it**, because it needs the rail to shrink: at 1280 the project rail and Nova's work column leave the thread about 640px.

**Deployed, and off by default.** Slice 5's and Slice 6's migrations are proven against a real PostgreSQL cluster the test harness creates itself, and as of 2026-09-17 the production database has both — applied through the Supabase MCP `apply_migration` (the token for the CLI workflow is still absent) and verified by reading the schema back rather than from the tool's own response. `src/types/database.ts` is regenerated. The account of it, including the one new security advisory the definer function raises, is [the audit's deployment note](../audits/2026-09-16-nova-first-restructure/README.md). The conversation lane is additionally behind `NOVA_CONVERSATION_ENABLED`, default off, and has never met a model: there is no conversation eval. A table that can hold a thread is not a decision that a founder should be talking to one.
