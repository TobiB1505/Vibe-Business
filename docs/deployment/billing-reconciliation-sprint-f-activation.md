...
**A once-flagged inconsistency, since reconciled**: the migration files this checklist and ADR 0042 name once carried different timestamps than what the remote database's own migration history recorded when `apply_migration` applied them — the same "manually-applied migrations may already exist on the remote database without matching local history" case rule 30 names, and the same fix Sprint 0058 had already precedented once on an unrelated migration: a bookkeeping-only `UPDATE supabase_migrations.schema_migrations SET version = '<local>' WHERE version = '<remote>' AND name = '<name>'`, per migration, run by the operator directly (blocked from this session's own tools by the same permission classifier that blocked Stage 3's row cancellation). Verified afterward: all 57 remote migration versions now match the 57 local files exactly, name and version both. See `docs/sprints/0068-billing-reconciliation-sprint-f-activation.md` for the full record.
