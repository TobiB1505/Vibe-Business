...
`credits/balance.ts` computes drift and `service.ts` logs it — then returns the drifted figure anyway. Its only caller, `getBillingBalance`, has no caller of its own outside the module, and `reconcileLotAllocation` has none at all. Sprint 0057 made the failure that produces drift loud (`MaterializationError`) rather than silent, which is the input a repairer would need; it deliberately did not invent one. *Designed, not yet built: [ADR 0041](decisions/0041-billing-reconciliation-authority.md)*, revised twice before implementation began. The first design (a nullable `materialized_at` on the ledger row, set "in the same call" as the aggregate) had a real crash-safety hole — the marker and the aggregate live on different tables, so setting them can never be atomic through PostgREST. Its replacement (a repair-only transactional function, called only from the account-balance read path) was itself found insufficient — proven, without needing any crash, to double-count a hot-path writer's delta if the writer's own materialization call lands after the repair commits. The current design shares one idempotent, per-row materialization primitive between the hot path and repair, so there is one mechanism rather than two that could disagree; four of five hot-path CAS writers move to it. **Sprint B0's schema landed** ([0058](sprints/0058-billing-reconciliation-b0-cutover.md) — the four marker columns, certified and backfilled against real Postgres) but the repair functions themselves (Sprint B1), the hot-path wiring (C/D) and activation (F) have not; `BILLING_REPAIR_ENABLED` does not exist and nothing reads the new columns yet. Unimplemented.
