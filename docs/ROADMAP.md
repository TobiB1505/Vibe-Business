...
`credits/balance.ts` computes drift and `service.ts` logs it — then returns the drifted figure anyway. Its only caller, `getBillingBalance`, has no caller of its own outside the module, and `reconcileLotAllocation` has none at all. Sprint 0057 made the failure that produces drift loud (`MaterializationError`) rather than silent, which is the input a repairer would need; it deliberately did not invent one. *Designed, not yet built: [ADR 0041](decisions/0041-billing-reconciliation-authority.md)*, revised once before implementation began — an initial marker-based design (a nullable `materialized_at` on the ledger row) was found to have a real crash-safety hole (the marker and the aggregate it describes live on different tables, so setting them can never be atomic through PostgREST) and was replaced with a narrowly-scoped, transactional repair function, called only from the account-balance read path. Unimplemented.
...
`lot-store.ts` logs and returns after ten contended attempts, on the argument that returning capacity is "the customer-favourable direction". It is the opposite: the customer keeps paying for Credits they can no longer spend, and nothing reconciles it. *Designed, not yet built: [ADR 0041](decisions/0041-billing-reconciliation-authority.md)* — the same revised repair-function mechanism as the account-level drift above, applied to `billing_credit_grants`/`billing_credit_allocations`, reusing the existing `reconcileLotAllocation` detector rather than replacing it. Unimplemented.
