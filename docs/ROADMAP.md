...
**A dead `queued`/`running` operation is now swept; an agent run paused for a question cannot resume, funded or not.**
`completeOperationRun` runs before `settleOperationBilling` (`business-audit/execution.ts`, and the same in action-plans and opportunities), so a sweeper keyed on a terminal operation would race settlement and produce a charge whose hold had been released. Sprint 0057 removed its planned sweeper on that evidence rather than shipping a heuristic. *Designed and implemented: [ADR 0041](decisions/0041-billing-reconciliation-authority.md) §P2, [Sprint 0063](sprints/0063-billing-reconciliation-p2-orphaned-hold-recovery.md)* — rejecting a lease/heartbeat model (a second liveness mechanism competing with Vercel Workflows, forbidden without its own ADR by rule 24) in favor of generalizing `expireStaleAgentExecution`'s already-proven "declare stale only at a read that already exists" pattern to `business_audit`/`opportunity_generation`/`action_planning`, wired into `getOperationStatus`. `needs_user` was found not to be a staleness question at all: it is now handled by an event, not a deadline — `pauseOperationForUser`'s two call sites release the hold immediately, with the reason resolved per call site (`cancelled_before_usage` for business-audit, `abandoned_with_usage` for agent-execution). Re-acquire-on-resume shipped for `business_audit` (`runInferenceStep`, keyed per pause cycle so a replayed resume cannot double-reserve). **It did not, and could not, ship for `agent_execution`**: traced directly during implementation, no code path in this repository transitions an agent run out of `needs_user_input` after its interrupt is answered — `answerExecutionInterrupt` only marks the interrupt row. A paused, credits-funded agent run therefore no longer leaks its hold, but it also cannot currently be resumed by anything; building that resume mechanism is a new, separate, larger gap this sprint surfaced rather than closed. A narrower residual gap also remains for `business_audit`: a crash between requeuing and re-acquiring leaves a `queued` operation with no valid hold, deliberately unswept for the same reason `queued` is never staleness-swept elsewhere — named, not fixed. The migration (`operation_runs.pause_cycle`) is deployed and verified against real Postgres; the application code reaches production through the normal PR/merge/Vercel deploy path.
