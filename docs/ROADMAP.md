...
**No test exercises a real Postgres constraint, and the environment to write one does not exist.**
...
What remains is the run itself, and it is blocked on infrastructure rather than on effort. A Supabase preview branch is refused because the organization is on the free plan (branching needs Pro); `supabase start` cannot pull images because `production.cloudfront.docker.com` and `pkg-containers.githubusercontent.com` are denied by the session's egress policy; production is excluded because the matrix writes deliberately conflicting financial rows into an append-only ledger. A direct `pg` connection is not a substitute — the production path is PostgREST, which is *why* the code compare-and-swaps. Closing this needs one of: the organization on Pro, an egress allowance for a container registry, or CI with Postgres and PostgREST service containers — the last being the only one that also makes it a regression gate.
