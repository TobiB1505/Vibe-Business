...
**Nothing repairs a drifted balance yet, on the lot side, and account-side repair itself has not run.**
`credits/balance.ts` computes drift and `service.ts` logs it. Sprint 0057 made an unmaterialized cache write fail loudly rather than silently, which named the shape of the input a repairer would need; it deliberately did not invent one. *Designed and now landed on the account side: [ADR 0041](decisions/0041-billing-reconciliation-authority.md)*, revised twice before implementation began. The first design (a nullable `materialized_at` on the ledger row, set "in the same call" as the aggregate) had a real crash-safety hole — the marker and the aggregate live on different tables, so setting them can never be atomic through PostgREST. Its replacement (a repair-only transactional function, called only from the account-balance read path) was itself found insufficient — proven, without needing any crash, to double-count a hot-path writer's delta if the writer's own materialization call lands after the repair commits. The shipped design shares one idempotent, per-row materialization primitive between the hot path and repair, so there is one mechanism rather than two that could disagree. **Sprint B0's schema landed** ([0058](sprints/0058-billing-reconciliation-b0-cutover.md) — the four marker columns, certified and backfilled against real Postgres), **Sprint B1's primitives landed** ([0059](sprints/0059-billing-reconciliation-b1-primitives.md) — the five idempotent functions, deployed and functionally proven against real data, nothing calling them yet), and **Sprint C wired the account side** ([0060](sprints/0060-billing-reconciliation-c-account-hotpath.md) — `postLedgerEntry`, `claimReservation` and `closeReservation` now call `materialize_ledger_entry`/`materialize_reservation_hold` instead of the retired compare-and-swap loops; `getBillingBalance` gains a call to `repair_account_balance` on detected drift, gated behind `BILLING_REPAIR_ENABLED`, still unset — so repair itself has not run against a real account yet). The lot-side wiring (Sprint D) and activation (Sprint F) have not landed; `BILLING_REPAIR_ENABLED` does not exist as a set variable anywhere. Unimplemented, on the lot side and for activation.
...
**`returnToLot`'s two real callers no longer strand capacity under contention, but nothing repairs a lot yet and no read path exists to trigger it.**
`lot-store.ts` used to log and give up after ten contended attempts, on the argument that returning capacity is "the customer-favourable direction" — backwards, since the customer keeps paying for Credits nothing can spend. *Designed: [ADR 0041](decisions/0041-billing-reconciliation-authority.md)* — the same shared materialization primitive as the account-level drift above, applied to `billing_credit_grants`/`billing_credit_allocations`. **Sprint B0's schema landed** ([0058](sprints/0058-billing-reconciliation-b0-cutover.md)) and **Sprint D wired the two callers that have an allocation row to key off** ([0061](sprints/0061-billing-reconciliation-d-lot-side-hotpath.md)): `settleReservationAllocations` and `releaseReservationAllocations` now call `materialize_allocation_capacity` instead of running their own compare-and-swap loop, so a race between two settlements (or a settlement and a release) can no longer double-return capacity or exhaust a retry budget without giving it back. `allocateReservation`'s failure-unwind path — undoing a `takeFromLot` for a plan that never committed an allocation row — deliberately keeps the old CAS `returnToLot`, for the identical structural reason `takeFromLot` itself was never converted: there is no durable row yet for the primitive to lock. **Repair is still neither wired nor reachable**: `repair_lot_allocation` is deployed (Sprint B1) but has no caller, and unlike the account side there is no existing "lot balance read" to attach a gated `BILLING_REPAIR_ENABLED` call to the way `getBillingBalance` hosts the account-side one — `reconcileLotAllocation` (`lots.ts`) still has zero callers outside its own test. That read path does not exist yet and inventing one is unscoped. Unimplemented.
