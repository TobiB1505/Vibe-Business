...
`completeOperationRun` runs before `settleOperationBilling` (`business-audit/execution.ts:661/672`, and the same in action-plans and opportunities), so a sweeper keyed on a terminal operation would race settlement and produce a charge whose hold had been released. Sprint 0057 removed its planned sweeper on that evidence rather than shipping a heuristic. *Designed, not yet built: [ADR 0041](decisions/0041-billing-reconciliation-authority.md)* rejects a lease/heartbeat model (a second liveness mechanism competing with Vercel Workflows, forbidden without its own ADR by rule 24) and instead generalizes `expireStaleAgentExecution`'s already-proven "declare stale only at a read that already exists" pattern to every operation family — for a genuinely dead workflow (`queued`/`running`). A `needs_user`-shaped instance of this same gap was also found and, on reconsideration, resolved differently: a paused operation is not a staleness question at all, because nothing should be held reserved during an indefinite human wait in the first place — see ADR 0041 for the release-on-pause / re-reserve-on-resume design. Unimplemented.
...
`service.ts` skips `postLedgerEntry` when `actualCredits === 0`, because the `credit_delta <> 0` CHECK forbids a zero-delta row. So the retry cannot find an existing charge, falls through, and refuses with `reservation_not_active`. *Designed, not yet built: [ADR 0041](decisions/0041-billing-reconciliation-authority.md)* — `settleReservation` should check `reservation.status === "settled"` directly, before consulting the ledger, since the reservation row (unlike a ledger row) is guaranteed to exist for a zero-credit outcome too. Unimplemented.
