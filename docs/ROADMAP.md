...
**Settling and releasing one reservation at once charges against a cancelled hold, every time.**
Measured by [Sprint 0057 E2b](sprints/0057-e2b-ci-postgres-concurrency.md): 20 of 20 iterations of `settleOperationCredits` racing `releaseOperationCredits` ended `released` with the charge standing and the lot's capacity consumed. Not occasionally — always, and for a structural reason: settle does strictly more work before its close than release does, so release reaches `closeReservation` first every time. The money stays self-consistent (charge, ledger and lot all agree on the same 300) and E1 already made `settleReservation` *report* this as `charge_without_hold` rather than prevent it. It is here because nothing today calls both paths concurrently for one reservation, and the day something does, the outcome is deterministic rather than unlikely. Deciding which caller wins is a domain decision, not a test fix.
