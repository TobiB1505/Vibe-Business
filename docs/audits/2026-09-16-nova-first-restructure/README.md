...
## Deployment — 2026-09-17, after Slice 8

Slice 5's and Slice 6's migrations are applied to the production project. This is recorded here rather than in the sprints that wrote them, because it happened after both and the paragraphs above are what it corrects.

**How, and why not the CLI.** `SUPABASE_ACCESS_TOKEN` is still unset, so `pnpm db:status`, `pnpm db:push` and `pnpm db:types` still cannot run. The deployment went through the Supabase MCP `apply_migration` — the path [Sprint 0078](../../sprints/0078-evidence-pack-v4.md) took for the same reason, and the one rule 29 does not demote: it is neither the CLI workflow it prefers nor the SQL-Editor copy/paste it calls an emergency fallback. The project ref was derived from the `NEXT_PUBLIC_SUPABASE_URL` hostname rather than guessed (rule 32) and confirmed against the account listing; the unrelated project reachable from the same credentials was not touched (rule 33). History was read before anything was written (rule 30): the remote ended at `20260911102101`, exactly these two were missing, and `nova_threads` and `nova_messages` did not exist.

**Verified by reading the database, not by believing the tool.** `apply_migration` answered `{"success": true}` twice, which is the tool's account of its own work. What was then read back: `nova_messages` has exactly eight `NOT NULL` columns (`author`, `created_at`, `id`, `kind`, `project_id`, `sequence`, `thread_id`, `user_id`) and twenty-two CHECK constraints; four RLS policies stand across the two tables; `append_nova_conversation_turn` exists with `search_path = ''`, is executable by `authenticated` and is not executable by `anon`; and `authenticated`'s UPDATE on `nova_threads` is still column-limited to `status` and `last_read_sequence`. One tool result was wrong and would have been believed: `list_tables` reported every column as `NOT NULL`, which is why the nullability figure above comes from `information_schema.columns` instead.

**Two local filenames changed, because the remote assigned its own versions.** `apply_migration` stamps the migration with its own timestamp rather than the name it is given, so the remote recorded `20260917082310` and `20260917082327` where the files read `20260917021500` and `20260917030500`. Left alone, a future `supabase db push` would see two unapplied local migrations and re-run SQL that is already there. The files were renamed to match what the database recorded — rule 34 says the remote converges to the files, and the two histories naming the same migration differently is the drift that rule exists to prevent. `docs/ROADMAP.md` carried the only reference to the old names.

**`src/types/database.ts` was behind by more than these two.** Regenerating it added `nova_threads`, `nova_messages` and `append_nova_conversation_turn` as expected — and also `founder_profiles`, `action_plan_handoffs` and `record_action_plan_handoff`, which were deployed in early September and never regenerated. 248 insertions and no deletions: nothing in the remote schema had drifted away from what the file already described, which is the reassuring half of that finding.

**One new production advisory, and it is the argued exception rather than a surprise.** `get_advisors` now returns `authenticated_security_definer_function_executable` (WARN) for `public.append_nova_conversation_turn`, because `authenticated` may reach it through `/rest/v1/rpc/`. That is the exception `supabase/tests/lifecycle-authority.migration.ts` names by argument: its reach is bounded by ownership re-checked inside against `auth.uid()` through the project row, it takes no owner argument, and it refuses a caller with no session — so what a founder can do with it directly is write a sentence into *their own* transcript attributed to Nova, which is self-deception rather than access, because the transcript is never read back as authority. The linter cannot see that argument and should not be expected to. It is named here so the owner can overrule it: <https://supabase.com/docs/guides/database/database-linter?lint=0029_authenticated_security_definer_function_executable>. The other findings are pre-existing and unrelated — `rls_enabled_no_policy` on five service-role-only tables (INFO) and `auth_leaked_password_protection` (WARN, a dashboard setting). The performance advisor reports seven unused indexes among the new ones, which is what an empty table looks like.

**A third migration followed, and it is the one that makes the first two usable.** Slice 7's boundary (`20260917122549_nova_conversation_boundary.sql`) was applied the same day, on the owner's instruction, through the same path. Without it `nova_threads` grants `authenticated` no insert at all — so `ensureOpenThread`, which Slice 6 calls under the founder's *own* session, fails with `42501` in any project no run has ever finished in, and *New chat* has nothing to insert under. Both tables were **empty** when it ran, so the insert policy it revokes took no row with it and nothing was migrated. Verified the same way: the messages insert policy is gone and `authenticated` holds `SELECT` there and nothing else, the threads insert grant is exactly three columns, the read-marker trigger is on `nova_threads` while `nova_messages` still carries none, and the turn function keeps `search_path=""` with its three guards. `src/types/database.ts` regenerated byte-identical, which is the expected answer for a migration that changes policies, grants, a function body and a trigger. **No new advisory**: the read-marker function is `security invoker`, so the one WARN above is still the only one, and the performance advisor reports no `auth_rls_initplan`.

**And a fourth, on 2026-09-17, closing a race the first three left open.** `open_nova_thread` was applied through the same path, on the owner's instruction. `ensureOpenThread` was a read and then an insert across two PostgREST requests, so two tabs asking a first question together both found no open thread, both inserted, and split one conversation across two threads that each looked complete; no client-side fix reaches that, because PostgREST gives each request its own transaction. The function decides under `pg_advisory_xact_lock` keyed on the project. It is **`security invoker`** — the problem is ordering, not permission, so it deliberately adds no definer surface beside the one that argued for itself, and `get_advisors` confirms it: the same single WARN as before. Verified from `pg_proc` rather than from `{"success": true}` — `prosecdef` false, `search_path=""`, `(uuid, text, boolean)` returning `SETOF nova_threads`, `EXECUTE` to `authenticated` and `service_role` and to neither `anon` nor `public` — and `nova_threads`' grants read back unchanged, which is the claim that matters: the callers keep exactly the reach they had. The platform assigned version `20260917142841` rather than the file's own `20260917150000`, so the file was renamed to it (rule 34); it still sorts after the boundary migration. `src/types/database.ts` gained the function and nothing else.

**What this does not mean.** The conversation lane is still behind `NOVA_CONVERSATION_ENABLED`, default off, and still has no eval. The tables can now hold a thread; nothing has decided that a founder should be talking to one.
