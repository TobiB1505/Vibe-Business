# Sprint 0155 — The box around the only thing on the screen

**Date:** 2026-09-08
**Decision:** none. The design pass over the four public auth routes, on the logic Sprint 0154 fixed.
...
**The form-level error has a form-level home.** Sprint 0154 recorded this and left it: *"Enter your email and password"* and *"We couldn't reach the server"* both rendered as a `Field` error on **password**, under one of the two fields they were not about — which tells a reader the other one is fine. `FormError` is a `role="alert"` block between the fields and the submit, described by both inputs.
