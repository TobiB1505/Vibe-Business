# Sprint 0162 — The question behind the dropdown
...
Both controls are at x=329 with the same height in the current build. The screenshots sent with Sprint 0161 were taken *before* the `items-start` change and showed the old right-aligned disconnect. Measured rather than argued, because "already fixed" is a claim worth checking before making.
