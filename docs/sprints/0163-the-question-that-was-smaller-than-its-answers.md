# Sprint 0163 — The question that was smaller than its answers

**Date:** 2026-09-08
**Decision:** none. A correction to Sprint 0162, including one of its own mistakes.
...
**`swipes between Moves while keeping details below`** failed by 7px under a full parallel run and passed three times in isolation. Not the font mechanism from Sprint 0161 — the opposite direction: this measures right after a swipe, and a sliding carousel track is momentarily wider than its viewport. The file already had `settledBox` for waiting out exactly that movement; the assertion uses it now rather than being lucky.
