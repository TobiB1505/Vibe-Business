...
3. **robots.txt / sitemap false positive.** Both surfaces matched *any* file named `robots.*` or `sitemap.*` anywhere in the tree, so this repository's own robots.txt and sitemap **parsers** (`src/modules/live-product-intelligence/`) were reported as the product serving `/robots.txt` and `/sitemap.xml` — directly contradicting the live crawl, which correctly reported both missing. A downstream opportunity was generated whose stated problem was false. Both surfaces now match only locations a framework actually serves from: a static directory (`public/`, `static/`), the repository root, or the Next.js App Router metadata convention (`app/robots.ts`, `src/app/sitemap.ts`, including route groups), each optionally under a monorepo workspace. `ANALYZER_VERSION` moved to `repo-intelligence-v2` so stored snapshots carrying the old rule are not reused.
