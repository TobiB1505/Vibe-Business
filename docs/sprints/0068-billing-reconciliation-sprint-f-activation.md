...
**The local/remote migration-version mismatch remains unreconciled**, flagged in `docs/deployment/billing-reconciliation-sprint-f-activation.md` — the three migration files' local timestamps do not match what `apply_migration` recorded remotely for the same migrations. Worth a deliberate `pnpm db:status` pass before any future migration touches these tables. **Recording drift and the two preconditions Sprint 0067 named** (a settlement retry's differing amount silently discarded; `refundCharge`'s dormancy) remain exactly as that sprint left them — genuinely out of scope for activation, not reopened here. **No new test, no new code.** This sprint is an operational record, not an implementation sprint.
