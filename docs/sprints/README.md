...
- [Sprint 13A — Execution Capability Expansion](0013a-execution-capability-expansion.md) — **PARTIAL**: the capability registry and completeness contract are done (a capability can no longer ship without a rationale, an outcome profile or a bounded write budget), and a deliberate regression exposed that the causal-language guard did not cover forward-looking promises. None of the three new capabilities are built; pre-flight found CTA would be `not_applicable` and pricing `needs_user_input` on this repository
