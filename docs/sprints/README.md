...
- [Sprint UI-2 (Part 1) — Workspace Read Models](0016-ui2-workspace-read-models.md) — the three boundaries the route split needs: a secure, project-scoped read path for the audit log (Activity shows real events for the first time), the prepared-change assembly extracted out of the project page, and a project-level impact read model (**Phases A–C complete; Phase D, the route split, deliberately not started — a partial split is a regression**)
