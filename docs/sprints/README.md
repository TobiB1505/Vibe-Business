...
- [Sprint 12B — Business Outcome Measurement Foundation](0012b-business-outcome-measurement.md) — the third and last question in the trust loop, built around what it refuses to say: business impact requires metric evidence, no evidence is not negative impact, and an observed change is never a caused one (Implemented and mutation-validated — migration written but **not deployed**; the real dogfood's honest result is `source-required`, because Vibe has no analytics connector and none was built to make it green)
