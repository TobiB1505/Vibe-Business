...
- **VB-008's limit applies to customer-initiated starts only.** Putting it in the single funnel covered every start path, which was its strength and also its cost: the billing concurrency gate drives one project sixty times on purpose, and a per-project window refuses exactly that. A per-project limit and a single-project race gate cannot both be satisfied, so the resolution is the threat model rather than a looser ceiling — `initiatedBy` is required at all fifteen call sites, and only the harness says `system`. A test pins that no service does, so the exemption cannot widen quietly.
