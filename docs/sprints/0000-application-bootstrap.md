...
- **Superseded: the magic-link auth flow described above is gone.** The `/auth/callback` PKCE `exchangeCodeForSession(code)` route turned out to be the root cause of a production sign-in bug (users landing on `/login?error=auth`) and was replaced by a `token_hash` + `verifyOtp` flow at `/auth/confirm` — which was itself then replaced by email + password (`supabase.auth.signInWithPassword`/`signUp`/`signOut`) before shipping, chosen for deterministic development/E2E testing without depending on Supabase's email rate limits. Neither `/auth/callback` nor `/auth/confirm` exist in the codebase anymore. See [src/modules/auth/README.md](../../src/modules/auth/README.md) ("Migration history") for the full sequence and why.
