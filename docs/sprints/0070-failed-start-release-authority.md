...
> **[2026-08-23] That sentence was false when written, and the CI run of this very sprint said so.** The scenario it names had never executed. Its fixture updated `operation_runs` to `status = 'completed'` without a `result_id`, which `operation_runs_completed_has_result` rejects — so PostgreSQL refused the write on the first iteration of every run, and the suite failed with `new row for relation "operation_runs" violates check constraint`. Run #14 of the `Billing concurrency` workflow, the run for this sprint's own pull request, is red with exactly that error. The claim above was written from a local `pnpm test` — which does not execute `*.concurrency.ts` at all — and the CI job that does was never opened. The FakeDatabase proof in the paragraph above it is unaffected and stands. Sprint 0075 fixes the fixture (a real finalizer always writes the agent run's id) and gets the gate green; only then does this scenario become a proof of anything. The original text is left as written, per rule 83.
