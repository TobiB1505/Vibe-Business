# Sprint 0060 — ADR 0042 Sprint C: the account-side hot path
...
Three of `credits/store.ts`'s five compare-and-swap writers are retired, replaced by `.rpc()` calls onto the ADR 0042 §P3 primitives Sprint B1 deployed:
...
`credits/lot-store.ts` is untouched — `returnToLot`'s stranded-capacity gap is Sprint D. `BILLING_REPAIR_ENABLED` is referenced but not set anywhere, so `repair_account_balance` has not been invoked against a real account by this deploy. Activation — verifying the drain conditions ADR 0042 §P3's Rollout section names and setting the flag — is Sprint F, and remains a separate, deliberate operator action.
