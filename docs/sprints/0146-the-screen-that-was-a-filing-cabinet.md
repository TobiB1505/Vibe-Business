# 0146 — The screen that was a filing cabinet
...
**Deleted:** `signal-card.tsx`, `product-card.tsx`, and the attention panel added four hours earlier in Sprint 0145. **Extracted:** `ScoreRing` and `ProductMark`, which were private to the deleted files and are now used at two sizes each.
