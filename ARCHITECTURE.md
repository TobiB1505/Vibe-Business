...
**[Accepted — ADR 0109]** A **second, separate contract** sits beside it: `AIToolCallingProvider` performs exactly one tool-calling turn (`countToolCallingInputTokens`, `generateWithTools`) and is implemented by the same adapter. `StructuredRequest` is unchanged and still has no field a tool could arrive through. Nothing in production calls the new contract at HEAD; its only caller is the seam pilot in `src/modules/business-agent/pilot/`, whose paid comparison of native tool calling against a structured-output action loop selected the native seam — [ADR 0109](docs/decisions/0109-the-business-agent-is-the-orchestrator.md) §"The paid run" holds the numbers and the two defects the selected seam carries into the first slice. The second exception to [rule 41](CLAUDE.md) is written; no production caller exists yet.
...
| [0109](docs/decisions/0109-the-business-agent-is-the-orchestrator.md) | The Business Agent is the orchestrator: one durable tool-calling turn under Nova's name, a second bounded exception to rule 41, founder-pressed controls for every consequential action (Accepted; the provider seam is built and the paid comparison selected native tool calling — acceptance covers the architecture and the seam, not yet the agent's behaviour) | §3.6, Web surface |
