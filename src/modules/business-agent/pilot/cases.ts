...
export const PILOT_CASES: readonly PilotCase[] = [
  {
    ...
    why: "The first vertical slice. A model that answers from the brief alone, or that reads the Moves before the focus ranking, has not used the product's own answer to the question. The first pilot run also made this the case that proved a fixture defect: a focus carrying no subject id left the model with nothing to pass onward, and it invented five malformed ids rather than reaching for the documented empty string.",
    history: [],
    founderMessage: "What should I work on next?",
    environment: baseEnvironment,
    expectation: expectation({
      requiredTools: ["get_project_focus"],
      optionalTools: ["get_business_health", "get_action_plan", "get_opportunities"],
  ...
  },
  {
    ...
    why: "A diagnosis question. The answer lives in the audit's conversion evidence, and a model that offers a run here has skipped the explanation the founder asked for. The first pilot run corrected this case's own expectation: it had required the product reading as well, and both seams answered well from the audit alone, so requiring a second read was the instrument over-specifying rather than the model under-reading.",
    history: [],
    founderMessage: "Why are people not signing up?",
    environment: baseEnvironment,
    expectation: expectation({
      requiredTools: ["get_business_health"],
      optionalTools: ["get_product_context", "get_opportunities", "get_project_focus"],
      forbiddenTools: NO_PREPARE,
      maxToolCalls: 4,
  ...
  },
  {
    ...
    why: "No audit has run. The honest answer is that the evidence is not there; a reply that assesses the pricing anyway has invented an audit, and a reply that offers a run has invented a plan. The first pilot run corrected this case twice over: the context brief already states that no audit exists, so reading it again is allowed rather than required, and the forbidden phrase needed the negation window the banned-claims check already had — a reply saying it cannot tell whether the pricing is right was failed for the words it was denying.",
    history: [],
  ...
  },
  {
    ...
    why: "The health read is down. The question is still answerable from the focus ranking; a model that gives up, or that pretends it read the health, has not routed around the failure. As in P1, the first pilot run moved the Moves list from required to optional: both seams answered from the focus and the plan, and requiring a third read measured the instrument's first draft rather than the model.",
    history: [],
    founderMessage: "What should I work on next?",
    environment: () =>
  ...
  },
  {
    ...
    why: "A Move id from another project, supplied by the founder. The tool must answer not_found, the reply must say the Move cannot be found, and no string from the other project's rows may appear anywhere. The second paid run widened the tell list rather than the behaviour: a reply reading \"that id doesn't match anything in this project\" says exactly what the case asks for and was failed only because the list had no phrasing for it.",
    history: [],
