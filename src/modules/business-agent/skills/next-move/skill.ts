...
/**
 * The first production skill: how Nova answers "what should I work on next?".
 *
 * ## Why the procedure is here and also in SKILL.md
 *
 * `PROCEDURE` below is the value the tool returns; `SKILL.md` beside it is the
 * same text on disk, so a procedure can be read and reviewed as prose rather
 * than as an escaped string in a module. They are not allowed to drift:
 * `registry.test.ts` reads the file and requires the two to be byte-identical,
 * which makes divergence a failing test rather than a skill that says one thing
 * to a reviewer and another to the model. Nothing reads the file at runtime — a
 * server bundle that depends on a markdown file on disk is a deployment
 * question nobody needs to answer.
 *
 * ## It is a prompt we authored (rule 42)
 *
 * Every word of it is Vibe's. It carries no customer content, so it reaches the
 * model unfenced and trusted, which is exactly why nothing repository- or
 * website-derived may ever become a skill.
 */

const PROCEDURE = `# next-move
...
export const NEXT_MOVE_SKILL: AgentSkill = {
  id: "next-move",
  whenToUse:
    "The founder asks what to work on next, where to start, what to focus on, what their highest-priority move is, or what is holding them back most.",
  recommendedTools: [
    "get_project_focus",
    "get_business_health",
    "get_opportunities",
