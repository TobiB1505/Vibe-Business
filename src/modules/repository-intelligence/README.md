...
Turns a connected GitHub repository into a versioned, evidence-carrying snapshot of what the project _is_ — stack, infrastructure signals, routes, business surfaces — with **no AI involved**.
...
| `candidates.ts`   | High-value file _discovery_ (free, from the tree) vs. _fetching_. Two families are downloaded and no others: dependency manifests, because a dependency list cannot be inferred from a filename, and a short named list of stylesheets, because a design token cannot be either. |
