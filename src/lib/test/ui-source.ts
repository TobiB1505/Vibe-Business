...
/**
 * A path relative to `src/`, because the panels these suites cover no longer
 * share one directory: the gate panels are `features/agent/`, the audit's are
 * `features/health/`, and the routes that mount them are still under
 * `app/app/projects/[projectId]/` (ADR 0109, Slice 2). A single base directory
 * would have to be a guess, and a guess that resolves to the wrong file of the
 * same name is exactly the silent pass this module exists to prevent.
 */
