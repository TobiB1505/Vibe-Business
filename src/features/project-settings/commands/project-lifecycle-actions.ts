...
export type DisconnectProjectFailure = "project_not_found" | ProjectWorkBlocked | "detach_failed";
 ...
 */
export type DisconnectProjectActionState = { ok: false; error: DisconnectProjectFailure } | null;
...
export type DeleteProjectActionState = { ok: false; error: DeleteProjectFailure } | null;
 ...
 */
export async function deleteProjectAction(projectId: string): Promise<DeleteProjectActionState> {
  const session = await requireSession();
  const supabase = await createClient();
