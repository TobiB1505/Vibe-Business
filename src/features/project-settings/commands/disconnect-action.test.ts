...
const { disconnectProjectAction } = await import("./project-lifecycle-actions");
...
function invoke() {
  return (
    disconnectProjectAction as (
      p: string,
      ...rest: unknown[]
    ) => ReturnType<typeof disconnectProjectAction>
...
describe("failure: not_found", () => {
  beforeEach(() =>
    disconnectProjectMock.mockResolvedValue({ ok: false, reason: "project_not_found" }),
  );
