...
import { getOperationStatus, startBusinessAuditOperation } from "@/modules/operations/service";
...
export async function getOperationStatusAction(
   ...
   */
  if (isTerminal(operation.status)) revalidatePath(projectPath(projectId));
