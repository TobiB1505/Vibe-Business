...
const HERE = join(process.cwd(), "src", "features");
...
describe("actions the plan screen offers on a stale plan", () => {
  it.each(["plan/commands/handoff-action.ts", "plan/commands/founder-action-attestation.ts"])(
  ...
  it("keeps the gate where answering a stale question would be wrong", () => {
     ...
     */
    expect(read("founder-input/commands/founder-input-action.ts")).toContain("staleness.length");
  });
  ...
  it("asks the attestation gate about handoffs, not just about the step", () => {
     ...
     */
    const source = read("plan/commands/founder-action-attestation.ts");
