...
/*
 * The component and its command sit in two layers now: the screen is still a
 * route file under `(account)/settings`, and the action it binds moved to
 * `features/account/commands` with the rest of the account's commands
 * (ADR 0109, Slice 3). Two constants rather than one, because a single base
 * would have to be a guess about which layer a file is in.
 */
const SCREEN = join(process.cwd(), "src/app/app/(account)/settings");
const COMMANDS = join(process.cwd(), "src/features/account/commands");
 ...
 */
const src = readFileSync(join(SCREEN, "delete-account.tsx"), "utf8").replace(/\s+/g, " ");
const action = readFileSync(join(COMMANDS, "delete-account-actions.ts"), "utf8");
