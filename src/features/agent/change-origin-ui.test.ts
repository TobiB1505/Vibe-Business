...
describe("the origin says what was asked for, never what was achieved", () => {
  const src = source("features/agent/change-origin.tsx");
  const copy = renderedCopy("features/agent/change-origin.tsx");
...
describe("the card shows one answer to why, not two", () => {
  const section = source("features/agent/change-meaning.tsx");
  ...
  it("links to the Move rather than restating it", () => {
    const backlink = source("features/agent/change-origin.tsx");
  ...
  it("keeps it in the meaning slot, above the git identity", () => {
     ...
     * asserted.
     */
    const route = source("app/app/projects/[projectId]/agent/page.tsx");
