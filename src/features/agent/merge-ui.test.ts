...
describe("the merge panel offers merging and nothing more (§15, §25)", () => {
  const src = source("features/agent/merge-panel.tsx");
  ...
  it("takes the deployment claim from the server card rather than asserting it", () => {
    ...
    expect(source("app/app/projects/[projectId]/../../../../modules/merge/view.ts")).toMatch(/deploymentVerified: false/);
  });
});
...
describe("the preview is absent for a change with nothing to look at", () => {
  const actions = source("features/agent/agent-stage-actions.tsx");
