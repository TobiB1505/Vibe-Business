...
import {
  ...
  getFounderInputRequestForInterrupt,
...
export async function startAgentRunAction(
  ...
  if (!persisted.ok) {
    return {
      ok: false,
      error: persisted.error === "project_not_found" ? "project_not_found" : "spec_not_persisted",
    };
  }
...
export type ResolveRuntimeFounderInputState = { ok: true } | { ok: false; message: string } | null;
...
export async function resolveAgentFounderInputAction(
  ...
  if (!persisted.ok) {
    return {
      ok: false,
      message:
        "Your answer was saved, but the new execution could not be prepared. Start the step again from Action Plan.",
    };
  }
  ...
  if (outcome.kind === "failed") {
    return {
      ok: false,
      message:
        "Your answer was saved, but the new execution could not start. Start the step again from Action Plan.",
    };
  }
  const agentHref = agentMoveHref(
    projectSectionHref(projectId, "agent"),
